...
public class MapLoader {
	...
	public void load(String path) {
		try {
			String map = IO.readToString(IO.openAsset(path));
			...
			for (int i = 0; i < maparr.length(); i++) {
				...
				Layer layer = new Layer(jlayer.getInt("id"),
						(float) jlayer.getDouble("parallax"), jsprites.length());
				layers.add(layer);
				for (int j = 0; j < jsprites.length(); j++) {
					JSONObject jsprite = jsprites.getJSONObject(j);
					TextureRegion tex = (TextureRegion) SevenGE
							.getAssetManager().getAsset(
									jsprite.getString("textureRegion"));
					Sprite sprite = new Sprite(tex);
					sprite.setScale((float) jsprite.getDouble("scaleX"),
							(float) jsprite.getDouble("scaleY"));
					sprite.setRotation((float) Math.toRadians(jsprite
							.getDouble("rotation")));
					sprite.setCenter((float) jsprite.getDouble("x"),
							(float) jsprite.getDouble("y"));
